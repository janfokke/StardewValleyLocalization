...
namespace XnbConvert.Compression
{
    internal class LzxDecoder
    {
        ...
        public LzxDecoder(int window)
        {
            ...
            for (var i = 0; i < wndsize; i++) m_state.window[i] = 0xDC;
            ...
            m_state.PRETREE_table =
                new ushort[(1 << LzxConstants.PRETREE_TABLEBITS) + (LzxConstants.PRETREE_MAXSYMBOLS << 1)];
            m_state.PRETREE_len = new byte[LzxConstants.PRETREE_MAXSYMBOLS + LzxConstants.LENTABLE_SAFETY];
            m_state.MAINTREE_table =
                new ushort[(1 << LzxConstants.MAINTREE_TABLEBITS) + (LzxConstants.MAINTREE_MAXSYMBOLS << 1)];
            m_state.MAINTREE_len = new byte[LzxConstants.MAINTREE_MAXSYMBOLS + LzxConstants.LENTABLE_SAFETY];
            m_state.LENGTH_table =
                new ushort[(1 << LzxConstants.LENGTH_TABLEBITS) + (LzxConstants.LENGTH_MAXSYMBOLS << 1)];
            m_state.LENGTH_len = new byte[LzxConstants.LENGTH_MAXSYMBOLS + LzxConstants.LENTABLE_SAFETY];
            m_state.ALIGNED_table =
                new ushort[(1 << LzxConstants.ALIGNED_TABLEBITS) + (LzxConstants.ALIGNED_MAXSYMBOLS << 1)];
            m_state.ALIGNED_len = new byte[LzxConstants.ALIGNED_MAXSYMBOLS + LzxConstants.LENTABLE_SAFETY];
            /* initialise tables to 0 (because deltas will be applied to them) */
            for (var i = 0; i < LzxConstants.MAINTREE_MAXSYMBOLS; i++) m_state.MAINTREE_len[i] = 0;
            for (var i = 0; i < LzxConstants.LENGTH_MAXSYMBOLS; i++) m_state.LENGTH_len[i] = 0;
        }
        ...
        public int Decompress(Stream inData, int inLen, Stream outData, int outLen)
        {
            ...
            var window = m_state.window;

            var window_posn = m_state.window_posn;
            var window_size = m_state.window_size;
            var R0 = m_state.R0;
            var R1 = m_state.R1;
            var R2 = m_state.R2;
            uint i, j;
            ...
            if (m_state.frames_read++ < 32768 && m_state.intel_filesize != 0)
            {
                ...
                else
                {
                    ...
                    var curpos = (uint) m_state.intel_curpos;
